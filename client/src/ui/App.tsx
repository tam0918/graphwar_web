...
export function App() {
  ...
  const topBar = (
    <section className="gw-panel gw-topBar">
      {!inGame ? (
        <>
          <label className="gw-field" style={{ width: 320 }}>
            WebSocket URL
            <input className="gw-input" value={wsUrl} onChange={(e) => setWsUrl(e.target.value)} />
          </label>
          <label className="gw-field" style={{ width: 220 }}>
            Name
            <input className="gw-input" value={name} onChange={(e) => setName(e.target.value)} />
          </label>
          <button className="gw-btn gw-btnPrimary" onClick={connect} disabled={connected}>
  ...
  const lobbyScreen = (
    <div className="gw-lobbyLayout">
      <div className="gw-panel gw-stack">
        <h2>Lobby</h2>

        <div className="gw-row" style={{ alignItems: "end" }}>
          <label className="gw-field" style={{ flex: 1 }}>
            room name
            <input
              className="gw-input"
            value={newRoomName}
          ...
          <label className="gw-field">
            ...
            <select
              className="gw-select"
              value={newRoomPreset}
              onChange={(e) => setNewRoomPreset(e.target.value as MatchPreset)}
              disabled={!connected}
            >
          ...
          <button
            className="gw-btn gw-btnPrimary"
            onClick={() =>
              send({
                type: "room.create",
                name: newRoomName,
        ...
        <button className="gw-btn" onClick={() => send({ type: "lobby.listRooms" })} disabled={!connected}>
          Refresh rooms
        </button>

        <ul className="gw-list">
          {rooms.map((r) => (
            <li key={r.id} className="gw-listItem">
              ...
              <button className="gw-btn gw-btnPrimary" onClick={() => send({ type: "room.join", roomId: r.id })} disabled={!connected}>
                Join
              </button>
            </li>
          ))}
      ...
      <div className="gw-panel gw-stack">
        ...
        {room ? (
          <>
            ...
            <>
              {clientId && room.gameState === "lobby" && clientId === room.ownerClientId ? (
                <div className="gw-row gw-rowEnd">
                  <label className="gw-field">
                    preset
                    <select
                      className="gw-select"
                  ...
                  <button className="gw-btn" onClick={() => send({ type: "room.addBot" })} disabled={!connected}>
                    Add bot
                  </button>
                </div>
              ) : (
                ...
                <ul className="gw-list">
                  {room.players.map((p) => (
                    <li key={p.clientId} className="gw-listItem" style={{ justifyContent: "space-between" }}>
                      <div style={{ minWidth: 0 }}>
                        <strong>{p.name}</strong> {p.isBot ? <span className="gw-badge">bot</span> : null}{" "}
                        {p.ready ? <span className="gw-badge">ready</span> : <span className="gw-badge">not ready</span>}
                      </div>
                      {clientId === room.ownerClientId && room.gameState === "lobby" && p.isBot ? (
                        <button
                          className="gw-btn gw-btnDanger"
                ...
                <div className="gw-row">
                  <button
                    className={`gw-btn ${ready ? "gw-btnReady" : ""}`}
                    onClick={() => send({ type: "player.ready", ready: !ready })}
                    disabled={!connected}
                  >
                    {ready ? "Unready" : "Ready"}
                  </button>
                  <button className="gw-btn gw-btnPrimary" onClick={() => send({ type: "game.start" })} disabled={!connected}>
                    Start
                  </button>
                  <button
                    className="gw-btn"
                ...
                <div>
                  <h3>Chat</h3>
                  <div className="gw-row" style={{ flexWrap: "nowrap" }}>
                    <input
                      className="gw-input"
                      value={chatText}
                  ...
                  <div className="gw-chatLog" style={{ height: 260 }}>
                    {chat.map((c, idx) => (
                      <div key={idx} style={{ marginBottom: 8 }}>
                        <span className="gw-faint" style={{ fontSize: 12 }}>
          ...
          <div className="gw-faint">Join or create a room to play.</div>
        )}
      </div>
    </div>
  );
  ...
  const postGameScreen = !room || !lastGameOver ? null : (
    <div className="gw-panel gw-panelStrong gw-stack" style={{ maxWidth: 720 }}>
      <h2>Game Over</h2>
      <div style={{ fontSize: 16 }}>
        {lastGameOver.winnerTeam == null ? "Result: Draw" : `Winner: Team ${lastGameOver.winnerTeam}`}
      </div>
      {lastGameOver.winners.length ? (
        <div className="gw-muted" style={{ fontSize: 14 }}>
          Winners: {lastGameOver.winners.map((w) => w.name).join(", ")}
        </div>
      ) : null}
      <div className="gw-row">
        <button className="gw-btn gw-btnPrimary" onClick={() => setDismissedGameOverAt(lastGameOver.endedAt)}>
  ...
  const gameScreen = (
    <div className="gw-gameLayout">
      <div className="gw-gameMain">
        <div className="gw-panel gw-row" style={{ justifyContent: "space-between", alignItems: "baseline" }}>
          <h2>Game</h2>
          <div className="gw-muted" style={{ fontSize: 12 }}>
      ...
      <div className="gw-gameSide">
        ...
        <div className="gw-panel gw-stack">
          ...
          <label className="gw-field">
            Function
            <input className="gw-input" value={functionString} onChange={(e) => setFunctionString(e.target.value)} disabled={!connected} />
          </label>
        ...
        {room?.game?.difficulty === "practice" ? (
          <div className="gw-panel gw-stack">
            ...
            {hintThinking ? (
              <div className="gw-faint" style={{ fontSize: 12, marginTop: 6 }}>
                AI thinking: {hintThinking.attempt}/{hintThinking.maxAttempts}
              </div>
            ) : null}
          </div>
        ) : (
          <div className="gw-panel gw-faint" style={{ fontSize: 12 }}>
            Hard mode: hints disabled.
          </div>
        )}

        <button
          className="gw-btn gw-btnPrimary"
          onClick={() => send({ type: "game.fire", functionString })}
          disabled={!connected || (inGame && !isMyTurn)}
        >
          Fire
        </button>
